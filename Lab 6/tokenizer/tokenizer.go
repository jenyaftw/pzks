	...
	for idxNow < len(expression) {
		...
		case NewToken:
			...
			if token.Whitespace.Check(charNow) { // Check for whitespace
				...
				if idxNow == 0 && (!token.Symbols.Check(charNow) && string(charNow) != "(") && (!token.Symbols.Check(charNow) && string(charNow) != "(" && string(charNow) != "-") {
		...
		case NumericLiteral:
			if token.RealNumericDigits.Check(charNow) {
				...
				if len(tokens) > 0 && tokens[len(tokens)-1].Text == "-" {
					...
					} else if tokens[len(tokens)-2].Type == token.OperatorType || tokens[len(tokens)-2].Type == token.ParanthesesOpenType {
						tokens[len(tokens)-1].Type = token.UnaryOperatorType
						if len(errors) > 0 {
							errors = errors[:len(errors)-1]
						}
		...
		case SymbolName:
			if token.Symbols.Check(charNow) {
				...
				if len(tokens)-1 >= 0 && tokens[len(tokens)-1].Text == "-" {
					if len(tokens)-2 < 0 {
						tokens[len(tokens)-1].Type = token.UnaryOperatorType
					}

					if len(tokens)-2 >= 0 && (tokens[len(tokens)-2].Type == token.OperatorType || tokens[len(tokens)-2].Type == token.ParanthesesOpenType) {
						tokens[len(tokens)-1].Type = token.UnaryOperatorType
					}
				}
